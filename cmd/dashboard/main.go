...
var (
	...
	what           tracker.QueryType = tracker.QueryPageViews
	pos            int               = 0
	row            int
	current        []tracker.Metric
	currentTitle   string
	extraSelection int
	dateRangeMode  int = 0
)
...
func main() {
	...
	for {
		...
		case "p":
			renderPageViews()
		case "b":
			what = tracker.QueryBrowsers
			renderPie("Browsers")
		case "o":
			what = tracker.QueryOSes
			renderPie("OSes")
		case "c":
			what = tracker.QueryCountry
			renderPie("Countries")
		case "r":
			what = tracker.QueryReferrerHost
			...
			renderTable()
		case "t":
			if what == tracker.QueryPageViews {
				metrics, err := getMetric(tracker.QueryPageViewList)
				if err != nil {
					log.Fatal(err)
				}
		...
		case "z":
			...
			end = int64(tracker.TimeToInt(time.Now()))
			days := 30
			if dateRangeMode == 1 {
				days = 90
			...
			start = int64(tracker.TimeToInt(time.Now().Add(-24 * time.Duration(days) * time.Hour)))
...
func renderPageViews() {
	what = tracker.QueryPageViews

	ui.Clear()

	metrics, err := getMetric(tracker.QueryPageViews)
	if err != nil {
		log.Fatal(err)
	}
